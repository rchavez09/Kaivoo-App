...
import {
  ChevronDown, ChevronRight, Hash, FolderOpen, FileText,
  Plus, X, Smile, Trash2,
} from 'lucide-react';
...
const EntryHeader = ({ node, updateAttributes, editor, getPos: _getPos }: NodeViewProps) => {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 py-2 px-1 select-none group/header">
        {/* Collapse chevron */}
        <button
          onClick={handleToggleCollapse}
          className="h-5 w-5 flex items-center justify-center rounded hover:bg-secondary/80 transition-colors text-muted-foreground"
        ...
        {/* Topic pill */}
        {topicPath ? (
          <span className="topic-chip text-[10px] py-0.5 px-1.5 group inline-flex items-center gap-1 cursor-default">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button
                className="text-[10px] text-muted-foreground/50 hover:text-muted-foreground px-1.5 py-0.5 rounded hover:bg-secondary/50 transition-colors inline-flex items-center gap-0.5"
            ...
            <PopoverContent className="w-56 p-1 max-h-[250px] overflow-y-auto" align="end" sideOffset={4}>
        ...
        {/* Tag pills */}
        {visibleTags.map(tag => (
          <span key={tag} className="tag-chip text-[10px] py-0.5 px-1.5 group inline-flex items-center gap-0.5">
        ...
        {showTagInput ? (
          <input
            ...
            placeholder="tag"
            className="w-16 text-[10px] bg-secondary/50 rounded px-1.5 py-0.5 outline-none focus:ring-1 focus:ring-primary/30"
          />
        ) : (
          <button
        ...
        <Popover>
          <PopoverTrigger asChild>
            <button
              className={cn(
                'text-sm leading-none px-1 py-0.5 rounded hover:bg-secondary/50 transition-colors',
                !moodData && 'text-muted-foreground/40',
              )}
              aria-label={moodData ? `Mood: ${moodData.label}` : 'Set mood'}
            >
              {moodData ? moodData.emoji : <Smile className="w-3.5 h-3.5" />}
            </button>
          </PopoverTrigger>
        ...
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <button
              className="text-muted-foreground/0 group-hover/header:text-muted-foreground/40 hover:!text-destructive transition-colors ml-0.5"
