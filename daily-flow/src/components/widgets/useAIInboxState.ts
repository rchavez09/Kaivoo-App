...
export function useAIInboxState() {
  ...
  const buildTagContext = useCallback(() => tags.map((t) => ({ id: t.id, name: t.name })), [tags]);

  const processInput = useCallback(
    async (text: string, followUp?: { question: string; answer: string }) => {
  ...
  const processLinkUrl = useCallback(
    ...
    try {
      ...
      for (const s of selected) {
        if (s.type === 'subtask') {
          ...
          await logAction(
            'subtask_created',
            { parentTaskId: s.parentTaskId, parentTaskTitle: s.parentTaskTitle, title: s.title },
            input,
          );
          createdItems.push(`Subtask: ${s.title}`);
        } else if (s.type === 'task') {
          const topicIds = s.topicPath
            ? topicPathCache.get(normalizeTopicPath(s.topicPath.replace(/^\[\[|\]\]$/g, ''))) || []
            : [];
          ...
          if (capture) {
            await logAction(
              'capture_created',
              { captureId: capture.id, content: s.content.substring(0, 100), topicIds, tags: tagNames },
              input,
            );
    ...
    try {
      ...
      if (capture) {
        await logAction(
          'capture_created',
          {
            ...
            content: editedNote.title,
            topicIds,
            tags: tagNames,
            sourceUrl: editedNote.sourceUrl,
          },
          `Link: ${linkUrl}`,
        );
      ...
      for (const task of linkTasks.filter((t) => t.selected)) {
        ...
        if (created) {
          await logAction(
            'task_created',
            { taskId: created.id, title: created.title, topicIds: taskTopicIds },
            `Link: ${linkUrl}`,
          );
