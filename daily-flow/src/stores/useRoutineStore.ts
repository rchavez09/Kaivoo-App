  ...
  persist(
    (set, get) => ({
      ...
      updateRoutineGroup: (id, updates) =>
        set((state) => ({ routineGroups: state.routineGroups.map((g) => (g.id === id ? { ...g, ...updates } : g)) })),
      ...
      getWeeklyRoutineStats: (groupId, weekOffset = 0, weekStartsOn = 1) => {
        ...
        return days.map((day) => {
          ...
          return {
            date: dateStr,
            completed: allCompletions.filter((c) => targetRoutines.some((r) => r.id === c.routineId)).length,
