...
const initialTopics: Topic[] = [
  { id: 'topic-daily-notes', name: 'Daily Notes', description: 'Your daily journal entries by date', createdAt: new Date() },
...
export const useKaivooStore = create<KaivooStore>()(
  ...
  getTopicByName: (name) => {
    ...
    return topics.find(t => t.name.toLowerCase() === lowerName) || 
  ...
  getCapturesByTopic: (topicId) => {
    ...
    return captures.filter(c => 
      ...
      c.topicIds.some(id => childPageIds.includes(id))
    );
  },
  ...
  getJournalEntriesByTopic: (topicId) => {
    ...
    return journalEntries.filter(e => 
      e.topicIds.includes(topicId) || 
      e.topicIds.some(id => childPageIds.includes(id))
  ...
  getTasksByTopic: (topicId) => {
    ...
    return tasks.filter(t => 
      t.topicIds.includes(topicId) || 
      t.topicIds.some(id => childPageIds.includes(id))
    );
  },
  ...
  getRoutinesByGroup: (groupId) => {
    return get().routines.filter(r => r.groupId === groupId).sort((a, b) => a.order - b.order);
  },
  ...
  getRoutineCompletionRate: (date, groupId) => {
    const routines = groupId !== undefined
      ? get().getRoutinesByGroup(groupId)
      : get().routines;
  ...
  getWeeklySummary: () => {
    ...
    const routineCompletionRate = totalPossibleCompletions > 0 
      ? Math.round((totalRoutineCompletions / totalPossibleCompletions) * 100) 
      : 0;
